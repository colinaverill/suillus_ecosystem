...
d.fig <- d[,.(plant.resp,soil.resp,suillus,fert,grav_moist,Block)]
d.fig <- d.fig[complete.cases(d.fig),]


#Fit model.----
mod <- MCMCglmm(log10(soil.resp) ~ fert + suillus + log10(plant.resp)*suillus, random = ~Block, data = d.fig, pr = T)

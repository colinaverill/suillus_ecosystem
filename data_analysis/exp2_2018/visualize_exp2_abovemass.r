...
d <- readRDS(exp2_linear_analysis_aboveground.path)
mod <- d$model
sum <- d$detrend.sum
d <- d$data

#Get means and 95% CIs.----
colnames(sum)[3] <- 'above.mass'
mu <- sum$above.mass
...
diff.var <- list()
for(i in 1:1000){
  a.mu <- sum[sum$suillus == 1,]$above.mass
  a.sd <- sum[sum$suillus == 1,]$sd
  b.mu <- sum[sum$suillus == 0,]$above.mass
...
#save line.----
png('exp2_plot_abovemass.png',width = 10, height = 5, units = 'in', res = 300)
...
legend(c(' (-) EM','(+) EM'), x = 'bottomright', bty='n', y.intersp = 2, pch = c(1,16))
...
#add outer label.
mtext('Experiment 2: high particulate organic matter', side =3, outer = T, line = -1.75, adj = 0.05, cex = 1.5)
